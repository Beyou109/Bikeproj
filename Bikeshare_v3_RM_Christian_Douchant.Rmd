---
title: "Bike Share Data Project"
desc: "Analyses data from three cities that use bikeshare (Data for Chicago, Washington and New York)"
...
# Convert hour and minutes into integer scale for latter graphic interpretation
temptd <- data.frame(table(temp$data_hrmin))

# Number of minutes in a day 60*24 = 1440 for analysis purposes
rn <- c(1:1440)
...
Overall, the most common month is JUne, considering it is late spring and most of these cities in the more northern climate would expect higher ridership then (See Chart 1).  Also, this is the summer holidy period and we would expect more ridership to increase from April to June. 
=======
Overall, the most common month is June, considering it is late spring and most of these cities in the more northern climate would expect higher ridership then (See Chart 1).  Also, this is the summer holiday period and we would expect more ridership to increase from April to June.
>>>>>>> refactoring
...
The most common day of the week for all cities is Wednesday - the peek business day of the week.  Interestingly, that changes by June during the late spring as the distribution is more flat as more holiday visitors use bikes anytime of the week - see Charts 3 and 4 including summary tables.

### *What is the most common hour of day?*

The most common of hour of the day overall is between 8 and 9 am in the morning for all cities.  A breakdown of hours in a day by day of the week, shows this pattern is due to the usual workday (Mon to Friday) - pattern is two peeks for Morning and late afternoon.  The weekends (Saturday and Sunday) shows a rather more normal distribution with a peek at midday (See Charts 5 to #11)
...
||||||| merged common ancestors
  ...
  theme(plot.title = element_text(hjust = 0.5, size = 12), 
=======
  ggtitle("Number of Bike Riders Per Month (Jan to June 2017) - Chart 1")+
  theme(plot.title = element_text(hjust = 0.5, size = 12),
>>>>>>> refactoring
...
||||||| merged common ancestors
  ...
  theme(plot.title = element_text(hjust = 0.5, size = 11), 
=======
  ggtitle(Bike Use over 24 hour period by Hour - Chart 6")+
  theme(plot.title = element_text(hjust = 0.5, size = 11),
>>>>>>> refactoring
        axis.title.x.bottom = element_text(hjust = 0.5, size =11),
        axis.text.x = element_text(color = "black", size = 7, hjust = 0.5,
